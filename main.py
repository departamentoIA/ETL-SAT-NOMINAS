...
Features:
- Automatic resumption with JSON checkpoints.
- Manual resumption from a row using `--resume-row`.
- Make pause by creating the `pause.flag` file.
...
Usage:
python main.py --root-data-path "path" --tables tableA tableB
python main.py --root-data-path "path" --tables tableA tableB --resume-table tableA --resume-row 251
"""
...
ENGINE = create_engine(
    ...
    pool_pre_ping=True,
    connect_args={"timeout": 3000}  # For queries
)
